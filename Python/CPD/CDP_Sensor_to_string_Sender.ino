...
// Modified: 12/6/2022
//
//Purpose: to recieve a string of data, put it in a payload and send it. 
//
...
float Rnd;
const int INTERVAL_MS = 1000; //1000; 
int counter = 1;
bool sentOk = false;
//char Buffer[100];
char Buffer;
...
bool runSensor(void *) {
  ...
  //myChars[2] = 53;
    if (Serial.available())
    {
      String String_buffer = Serial.readString();
        for (int i=0; i<149; i++) {
    
    myChars[i] = String_buffer[i];
    }
  }

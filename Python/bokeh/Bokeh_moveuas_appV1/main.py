...
tabs_object = Tabs(tabs = [])
def generate_graphs():
    global active_sensors

    
    tabs_object.tabs.clear()
    for ii in active_sensors:
         print('this is it:')
         print(ii)
         #temp_tab = Panel(child = figure(title = box_group.labels[ii]), title = box_group.labels[ii])
         temp_tab = Panel(child = figure(title = ii), title = ii)

         tabs_object.tabs.append(temp_tab)
        
    #print(len(tabs_object.tabs))


    #This for loop turns them all into scatter plots, you can change the type of plot
    for ll in range(0, len(tabs_object.tabs)):
        #print(tabs_object.tabs)
        tabs_object.tabs[ll].child.scatter()
...
layout = layout([   [current_time_display], [up_time_display],[last_data_time_display], [selected],
              [box_group, down_menus], [Lock_Button],[Tables],[tabs_object]],sizing_mode='stretch_width')
